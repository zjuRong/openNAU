...
if (!file.exists("sample_mass_RC_QC_normalize_spoly3.csv")) {
  stop(" No 'sample_mass_RC_QC_normalize_spoly3.csv' file in this directory !")
}
data <- read.csv("sample_mass_RC_QC_normalize_spoly3.csv")
...
for (i in 1:m) {
  ...
  for (j in t:length(group))
  {
    ...
    ff <- paste0(group1, " vs ", group2, "++wilcox_all_4.csv")
    ...
    ff <- paste0(group1, " vs ", group2, "++wilcox_mean_4.csv")
...
for (i in 1:m) {
  ...
  for (j in t:length(group))
  {
    ...
    if (class(opls) == "try-error") {
      ...
      ff <- paste0(group1, " vs ", group2, "++oplsda_data_4.csv")
      write.csv(dd, ff, row.names = FALSE)
      summary <- data$summaryDF[, 1:4]
      ff <- paste0(group1, " vs ", group2, "++oplsda_summary_4.csv")
      write.csv(summary, ff, row.names = FALSE)
      ff <- paste0(group1, " vs ", group2, "++VIP_all_4.csv")
      write.csv(vip_data, ff,row.names=F)
      VIP <- vip_data[which(VIP > 1),]
      ff <- paste0(group1, " vs ", group2, "++VIP_mean_4.csv")
...
dd <- grep("VIP_mean_3.csv", file, fixed = TRUE)
if (length(dd) == 0) {
} else {
  file_popls <- file[dd]
  res <- c()
  for (f in file_popls)
  {
    ...
    ggr <- nn[1]
    wilcox <- read.csv(paste0(ggr, "++wilcox_mean_4.csv"))
  ...
  write.csv(peak_res, "DMC_wilcox_opls_mean_4.csv", row.names = FALSE)
}
...
dd <- grep("DMC_wilcox_opls_mean_4.csv", file, fixed = TRUE)
if (length(dd) > 0) {
  peak_mean <- read.csv("DMC_wilcox_opls_mean_4.csv", header = TRUE)
  ...
  write.csv(res, "DMC_wilcox_opls_mean_identify_4.csv", row.names = FALSE)
}
...
write.csv(data_res, "DMC_wilcox_opls_mean_identify_target_4.csv", row.names = FALSE)
...
write.csv(data_res, "DMC_wilcox_opls_mean_identify_enzyme_4.csv", row.names = FALSE)

cat("progress:90", file = output_log, sep = "\n", append = TRUE)


file.copy(from = "../../../R/process_4.Rmd", to = "process_4.Rmd", overwrite = TRUE)
render("process_4.Rmd", html_document(toc = TRUE, toc_depth = 3), "process_4.html")
