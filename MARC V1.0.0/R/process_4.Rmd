...
data_raw_QC <- read.csv("sample_mass_RC_QC_normalize_spoly3.csv")
...
file <- list.files()
dd <- grep("wilcox_all_4.csv", file, fixed = TRUE)
file_wilcox <- file[dd]
pi <- 1
gplot <- list()
...
dd <- grep("oplsda_data_4.csv", file, fixed = TRUE)
if (length(dd) == 0) {
  ...
  for (f in file_popls)
  {
    ...
    nn <- unlist(strsplit(f, "++", fixed = TRUE))
    ggr <- nn[1]
    summary <- read.csv(paste0(ggr, "++oplsda_summary_4.csv"))
...
dd <- grep("VIP_mean_4.csv", file, fixed = TRUE)
if (length(dd) == 0) {
...
<a href="DMC_wilcox_opls_mean_4.csv">Download data of differential metabolites</a>
...
if (length(dd) > 0) {
  peaks_mean <- read.csv("DMC_wilcox_opls_mean_4.csv", header = TRUE)
...
dd <- grep("DMC_wilcox_opls_mean_identify_4.csv", file, fixed = TRUE)
if (length(dd) > 0) {
  errm <- ""
  peaks_mean <- read.csv("DMC_wilcox_opls_mean_identify_4.csv", header = TRUE)
  res <- peaks_mean %>%
    group_by(extend.DB) %>%
...
```
<a href="DMC_wilcox_opls_mean_identify_4.csv">Download data of all database</a>
...
genelist <- read.csv("DMC_wilcox_opls_mean_identify_4.csv")
...
--> return NULL..."
    ...
    errm[i] <- ""
    pp <- paste0(ugg[i], "++DMC_wilcox_opls_mean_identify_pathway_4.csv")
    write.csv(result, pp)
...
```{r echo=FALSE,warning=FALSE}
file <- list.files()
dd <- grep("DMC_wilcox_opls_mean_identify_target_4.csv", file, fixed = TRUE)
if (length(dd) > 0) {
  errm <- ""
  data <- read.csv("DMC_wilcox_opls_mean_identify_target_4.csv")
...
```{r echo=FALSE,warning=FALSE}
file <- list.files()
dd <- grep("DMC_wilcox_opls_mean_identify_enzyme_4.csv", file, fixed = TRUE)
if (length(dd) > 0) {
  errm <- ""
  data <- read.csv("DMC_wilcox_opls_mean_identify_enzyme_4.csv")
...
## {-}
<b>`r errm`</b>
<a href="DMC_wilcox_opls_mean_identify_target_4.csv">Download data of target protein</a><br>
<b>`r errm`</b>
<a href="DMC_wilcox_opls_mean_identify_enzyme_4.csv">Download data of enzyme protein</a>
