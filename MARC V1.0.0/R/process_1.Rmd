...
file <- list.files()
dd <- grep("wilcox_all.csv", file, fixed = TRUE)
file_wilcox <- file[dd]
pi <- 1
gplot <- list()
for (f in file_wilcox)
{
  ...
  p <- ggplot(data, aes(logFC, lgFDR)) +
    ...
    ylim(min(data$lgFDR), max(data$lgFDR))
...
dd <- grep("oplsda_data.csv", file, fixed = TRUE)
if (length(dd) == 0) {
  ...
  for (f in file_popls)
  {
    ...
    nn <- unlist(strsplit(f, "++", fixed = TRUE))
    ggr <- nn[1]
    summary <- read.csv(paste0(ggr, "++oplsda_summary.csv"))
...
## The information for differential metabolites
```{r echo=FALSE}
file <- list.files()

dd <- grep("DMC_wilcox_opls_mean.csv", file, fixed = TRUE)
if (length(dd) > 0) {
  peaks_mean <- read.csv("DMC_wilcox_opls_mean.csv", header = TRUE)
  kable(peaks_mean[, 1:9])
}
```
...
dd <- grep("DMC_wilcox_opls_mean_identify.csv", file, fixed = TRUE)
if (length(dd) > 0) {
  errm <- ""
  peaks_mean <- read.csv("DMC_wilcox_opls_mean_identify.csv", header = TRUE)
  res <- peaks_mean %>%
    group_by(extend.DB) %>%
  ...
  ggplot(data = res, aes(x = extend.DB, y = n, fill = extend.DB), fill = extend.DB) +
    ...
    geom_text(mapping = aes(x = extend.DB, y = n, label = n), size = 3) +
...
```
<a href="DMC_wilcox_opls_mean_identify.csv">Download data of all database</a>
...
genelist <- read.csv("DMC_wilcox_opls_mean_identify.csv")
...
```{r echo=FALSE,warning=FALSE}
file <- list.files()
dd <- grep("DMC_wilcox_opls_mean_identify_target.csv", file, fixed = TRUE)
if (length(dd) > 0) {
  errm <- ""
  data <- read.csv("DMC_wilcox_opls_mean_identify_target.csv")
  dat <- data[, c("name", "Group", "general.name", "gene.name", "gene.function", "unport.ID", "genecard.ID")]
  kable(dat)
} else {
...
```{r echo=FALSE,warning=FALSE}
file <- list.files()
dd <- grep("DMC_wilcox_opls_mean_identify_enzyme.csv", file, fixed = TRUE)
if (length(dd) > 0) {
  errm <- ""
  data <- read.csv("DMC_wilcox_opls_mean_identify_enzyme.csv")
  dat <- data[, c("name.x", "Group", "general.name", "ECID", "name.y")]
...
## {-}
<b>`r errm`</b>
<a href="DMC_wilcox_opls_mean_identify_target.csv">Download data of target protein</a><br>
<b>`r errm`</b>
<a href="DMC_wilcox_opls_mean_identify_enzyme.csv">Download data of enzyme protein</a>
